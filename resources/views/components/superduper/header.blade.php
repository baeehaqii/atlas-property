        ...
        <div class="flex items-center justify-between gap-x-4 md:gap-x-8">
            ...
            <div>
                <a href="{{ route('home') }}" class="relative z-10 flex-shrink-0">
                    ...
                    @if ($brandLogo)
                        <img src="{{ Storage::url($brandLogo) }}" alt="{{ $brandName }}" class="w-auto h-10 md:h-12" />
                    @else
            ...
            <div class="menu-block-wrapper lg:static">
                <div class="fixed inset-0 z-40 menu-overlay bg-primary-900/70 backdrop-blur-sm lg:hidden"
                    style="display: none;"></div>
                <nav class="menu-block fixed top-0 right-0 bottom-0 w-[280px] text-[#B19244] md:w-[320px] bg-primary-600 dark:bg-primary-800 z-50 shadow-2xl overflow-y-auto transform translate-x-full transition-transform duration-300 lg:static lg:translate-x-0 lg:w-auto lg:bg-transparent lg:shadow-none lg:overflow-visible lg:dark:bg-transparent"
                    ...
                    <div class="flex items-center justify-between p-4 lg:hidden">
                        ...
                        <div class="text-2xl cursor-pointer mobile-menu-close text-primary-800 dark:text-white">&times;
                        </div>
                    </div>
                    ...
                    <ul class="p-4 text-lg site-menu-main lg:p-0 lg:flex lg:items-center lg:space-x-1">
                        @if ($menu)
                            @foreach ($menu->menuItems as $index => $item)
                                ...
                                <li
                                    class="nav-item mb-3 lg:mb-0 lg:relative {{ $hasChildren ? 'nav-item-has-children' : '' }}">
                                    <a href="{{ $item->url }}"
                                        class="nav-link-item rounded-lg flex items-center justify-between hover:text-primary-600 dark:text-white dark:hover:text-primary-200 header-nav-link font-medium py-2 lg:px-3 !text-[#B19244] lg:hover:bg-primary-600 lg:dark:hover:bg-primary-700 transition-colors {{ $hasChildren ? 'drop-trigger' : '' }}"
                                        @if ($item->target) target="{{ $item->target }}" @endif>
                                        <span>{{ $item->title }}</span>
                                        @if ($hasChildren)
                                            <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 ml-1 lg:h-5 lg:w-5"
                                    ...
                                    @if ($hasChildren)
                                        <ul class="sub-menu pl-4 mt-2 lg:absolute lg:left-0 lg:top-full lg:mt-1 lg:pl-0 lg:min-w-[200px] lg:bg-white lg:dark:bg-primary-800 lg:shadow-lg lg:opacity-0 lg:invisible lg:transform lg:translate-y-2 lg:transition-all lg:group-hover:opacity-100 lg:group-hover:visible lg:group-hover:translate-y-0 lg:z-20"
                                            ...
                                            @foreach ($item->children as $childIndex => $childItem)
                                                ...
                                                <li
                                                    class="sub-menu--item mb-2 lg:mb-0 {{ $hasGrandchildren ? 'nav-item-has-children' : '' }}">
                                                    ...
                                                    @if ($hasGrandchildren)
                                                        <ul class="sub-menu pl-4 mt-2 lg:absolute lg:left-full lg:top-0 lg:pl-0 lg:mt-0 lg:min-w-[200px] lg:bg-white lg:dark:bg-primary-800 lg:shadow-lg lg:opacity-0 lg:invisible lg:transform lg:translate-x-2 lg:transition-all lg:group-hover:opacity-100 lg:group-hover:visible lg:group-hover:translate-x-0"
                                                            id="{{ $submenuId }}">
            ...
            <div class="flex items-center gap-4 md:gap-6">
                <a href="admin/login" class="relative z-10 hidden sm:inline-block group">
                    <div class="flex items-center gap-x-2 px-4 py-2  text-sm font-medium transition-all duration-300 btn md:text-base bg-white/20 backdrop-blur-sm rounded-lg p-3 lg:p-4">
                    ...
                    <div class="absolute inset-0 -z-10 translate-x-[3px] translate-y-[3px] bg-[#B19244] transition-all duration-300 ease-linear rounded-xl group-hover:translate-x-0 group-hover:translate-y-0"></div>
                </a>
                ...
                <div class="block lg:hidden">
                    <button id="openBtn"
                        ...
                        <span
                            class="block w-6 h-0.5 bg-white dark:bg-white mb-1.5 transition-transform hamburger-line"></span>
                        <span
                            class="block w-6 h-0.5 bg-white dark:bg-white mb-1.5 transition-opacity hamburger-line"></span>
                ...
                if (window.innerWidth < 1024) {
                    ...
                    dropTriggers.forEach(trigger => {
                        trigger.addEventListener('click', function(e) {
                            if (window.innerWidth < 1024) {
                                ...
                                if (submenu) {
                                    const siblingMenus = parent.parentElement.querySelectorAll(
                                        '.sub-menu');
                    ...
                    goBack.addEventListener('click', function() {
                        ...
                        if (activeSubmenu) {
                            ...
                            if (activeSubmenu.parentElement.parentElement.classList.contains(
                                    'site-menu-main')) {
                                currentMenuTitle.textContent = '';
                                this.style.display = 'none';
                            } else {
                                const parentTrigger = activeSubmenu.parentElement.parentElement
